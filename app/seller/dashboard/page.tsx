...
export default function SellerDashboardPage() {
  ...
  const [productFilterTab, setProductFilterTab] = useState<"ACTIVE" | "ARCHIVED">("ACTIVE");
...
------------------------------`;
  ...
  const handleRestoreProduct = async (id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setSuccessToast(data.message || "Product listing restored to active store!");
        setTimeout(() => setSuccessToast(""), 5000);
      } else {
        setErrorToast(data.error || "Failed to restore product.");
        setTimeout(() => setErrorToast(""), 6000);
      }
    } catch (e: any) {
      console.error("Error restoring product:", e);
      setErrorToast("Network error. Could not restore product.");
      setTimeout(() => setErrorToast(""), 6000);
    }
  };
  ...
  const displayedProducts = productFilterTab === "ACTIVE" ? activeProducts : archivedProducts;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center p-1 bg-slate-100 rounded-xl border border-slate-200">
                ...
                <button
                  type="button"
                  onClick={() => setProductFilterTab("ARCHIVED")}
                  className={`px-3 py-1 rounded-lg text-xs font-bold transition flex items-center gap-1.5 ${
                    productFilterTab === "ARCHIVED"
                      ? "bg-[#0B0F14] text-[#C8A96B] shadow-xs"
                      : "text-slate-600 hover:text-slate-900"
                  }`}
                >
          ...
          <div className="bg-white rounded-3xl border border-[#E8E5DC] shadow-xs overflow-hidden">
            {displayedProducts.length === 0 ? (
              <div className="py-12 px-4 text-center">
                {productFilterTab === "ARCHIVED" ? (
                  <>
                    <Archive className="w-12 h-12 text-[#8A8F98] mx-auto mb-3" />
                    <p className="text-sm font-bold text-[#0B0F14]">No archived products</p>
                    <p className="text-xs text-[#8A8F98] mt-1 max-w-sm mx-auto">
                      Products that have historical customer orders will safely appear here when removed from your store.
                    </p>
                  </>
                ) : (
                  <>
                    <Package className="w-12 h-12 text-[#8A8F98] mx-auto mb-3" />
                    <p className="text-sm font-bold text-[#0B0F14]">No active products listed yet</p>
                        ...
                        <td className="py-3 px-4 text-right">
                          {p.status === "ARCHIVED" ? (
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-bold rounded-xl text-xs transition active:scale-98 shadow-xs"
      ...
      {shippingModalItem && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-lg w-full p-5 sm:p-6 space-y-5 shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto my-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
